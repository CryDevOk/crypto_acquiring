...
async def coins_txs_parser(transactions: list[dict], coins: dict[str, dict], logger: logging.Logger):
    """
...
async def native_txs_parser(block: dict, native_coin: dict, logger: logging.Logger):
    deposits = []
            ...
            if tx_info["type"] == "TransferContract":
                ...
                if recipient in variables.user_accounts_hex and sender not in variables.handler_accounts_hex:
                    if amount >= native_coin[Coins.min_amount.key]:
                        ...
                        logger.info(f"deposit less then minimum amount {unit['txID']}")
    return deposits
        ...
        try:
            ...
            if latest_trust_block > current_block:
                if latest_trust_block - current_block > Cfg.block_offset * Cfg.allowed_slippage:
                    # variables.slippage_block_gauge.set(latest_trust_block - current_block)
                    logger.warning(
                ...
                try:
                    ...
                    async with write_async_session() as session:
                        ...
                        coins = {web3_utils.to_hex_address(coin[Coins.contract_address.key]): coin for coin in resp if
                        ...
                        native_txs = await native_txs_parser(block, native_coin, logger)
